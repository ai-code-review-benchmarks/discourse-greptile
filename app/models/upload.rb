...
class Upload < ActiveRecord::Base
  ...
  def content
    ...
    if original_path.blank?
      Discourse.store.download!(self) { |f| File.read(f.path) }
    else
  ...
  def fix_image_extension
    ...
    begin
      ...
      original_path = Discourse.store.path_for(self)
      original_path = Discourse.store.download(self) { |f| f.path } if original_path.blank?
    ...
    begin
      path =
        if local?
          ...
          Discourse.store.download!(self) { |f| f.path }
        end
